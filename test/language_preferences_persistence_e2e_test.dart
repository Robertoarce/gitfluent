...
class MockAuthService implements AuthService {
  ...
  @override
  Future<void> updateUserProfile({String? firstName, String? lastName, String? profileImageUrl}) async => throw UnimplementedError();
  @override
  Future<void> deleteAccount() async => throw UnimplementedError();
  @override
  Future<bool> isPremiumUser() async => false;
  @override
  Future<void> updatePremiumStatus(bool isPremium) async => throw UnimplementedError();
  @override
...
class MockDatabaseService implements DatabaseService {
  ...
  @override
  Future<UserVocabularyItem> saveVocabularyItem(UserVocabularyItem item) async => item;
  @override
  ...
  @override
  Future<UserVocabularyStats?> getUserVocabularyStats(String userId, String language) async => null;
  @override
...
void main() {
  group('Language Preferences Persistence E2E Tests', () {
    ...
    test('language preferences persist across app restart for logged-in user', () async {
      ...
      final spanishLang = LanguageSettings.availableLanguages.firstWhere((l) => l.code == 'es');
      final germanLang = LanguageSettings.availableLanguages.firstWhere((l) => l.code == 'de');
      
      await languageSettings1.setTargetLanguage(spanishLang); // Italian -> Spanish
      await languageSettings1.setNativeLanguage(germanLang);  // English -> German
      ...
      expect(updatedUser?.preferences.supportLanguage1, equals('es')); // Spanish (unchanged)
      expect(updatedUser?.preferences.supportLanguage2, equals('fr')); // French (unchanged)
    ...
    test('language preferences persist locally for non-logged-in user', () async {
      ...
      final portugueseLang = LanguageSettings.availableLanguages.firstWhere((l) => l.code == 'pt');
      final frenchLang = LanguageSettings.availableLanguages.firstWhere((l) => l.code == 'fr');
      ...
      expect(languageSettings1.targetLanguage?.code, equals('pt')); // Portuguese
      ...
      expect(languageSettings2.targetLanguage?.code, equals('pt')); // Portuguese
    ...
    test('language preferences sync from database to local on login', () async {
      ...
      final italianLang = LanguageSettings.availableLanguages.firstWhere((l) => l.code == 'it');
      final englishLang = LanguageSettings.availableLanguages.firstWhere((l) => l.code == 'en');
      ...
      expect(languageSettings.supportLanguage2?.code, equals('vi')); // Vietnamese
    });
  });
}