...
class ChatService extends ChangeNotifier {
  ...
  ChatService(
      {required SettingsService settings, LanguageSettings? languageSettings})
      : _settings = settings,
    ...
    // Listen to language settings changes if provided
    if (_languageSettings != null) {
      _languageSettings!.addListener(_updateSystemPromptWithLanguages);
    }
  }
  ...
  void dispose() {
    ...
    if (_languageSettings != null) {
      _languageSettings!.removeListener(_updateSystemPromptWithLanguages);
    }
    super.dispose();
  }
  ...
  Future<void> _updateSystemPromptWithLanguages() async {
    try {
      ...
      final variables = {
        ...
        'native_language': _languageSettings!.nativeLanguage?.code ?? 'en',
        'support_language_1': _languageSettings!.supportLanguage1?.code ?? 'es',
