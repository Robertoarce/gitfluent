...
class SupabaseConfig {
  ...
  static String get serviceRoleKey {
    ...
    if (keyLength == 0) {
      debugPrint('WARNING: SUPABASE_SERVICE_ROLE_KEY is empty! This will prevent user creation.');
    } else if (keyLength < 10) {
      debugPrint('WARNING: SUPABASE_SERVICE_ROLE_KEY seems too short! It may be invalid.');
    } else {
  ...
  static void logConfigInfo() {
    ...
    debugPrint('SupabaseConfig: Tables: $usersTable, $vocabularyTable, $vocabularyStatsTable, $chatHistoryTable');
    debugPrint('SupabaseConfig: Anon key available: ${anonKey.isNotEmpty}');
    debugPrint('SupabaseConfig: Service role key available: ${serviceRoleKey.isNotEmpty}');
  }
} 