...
class UserService extends ChangeNotifier {
  ...
  void _initialize() {
    ...
    _authService.authStateChanges.listen((user) async {
      ...
      if (user != null) {
        // Fetch complete user data including language preferences and premium status
        try {
          debugPrint(
          ...
          if (dbUser != null) {
            ...
            try {
              // Ensure we use the user object that might have updated language preferences from the auth service
              final userId = await _databaseService.createUser(user);
              debugPrint(
                  'UserService: Created user in database with ID: $userId');
              // After creating, fetch it again to get the full updated user object
              final createdDbUser = await _databaseService.getUserById(userId);
              if (createdDbUser != null) {
                _currentUser = createdDbUser;
                debugPrint(
                    'UserService: Loaded created user from database: ${createdDbUser.email}');
              }
            } catch (e) {
          ...
          _currentUser = user; // Fallback to the user from auth service
        }
        notifyListeners();
      } else {
        _currentUser = null;
    ...
    try {
      ...
      if (authResult.success && authResult.user != null) {
        ...
        final user = User(
          ...
          targetLanguage: 'it', // Default target language
  ...
  Future<void> updateUserLanguagePreferences(User updatedUser) async {
    if (_currentUser == null) return;
    ...
    try {
      ...
      _error = 'Failed to update language preferences: $e';
      notifyListeners();
    }
  }
  ...
  Future<UserVocabularyStats?> getVocabularyStats({String? language}) async {
    ...
    try {
      return await _databaseService.getUserVocabularyStats(
        _currentUser!.id,
        language ?? _currentUser!.targetLanguage,
      );
    } catch (e) {
