...
class SupabaseAuthService implements AuthService {
        ...
        try {
          ...
          try {
            ...
            if (dbUser.isNotEmpty) {
              final isPremium =
                  (dbUser.isNotEmpty) ? dbUser['is_premium'] ?? false : false;
              _currentUser = _currentUser!.copyWith(isPremium: isPremium);
              debugPrint(
    ...
    try {
      ...
      if (session != null) {
        ...
        try {
          ...
          final isPremium = (dbUser.isNotEmpty)
              ? dbUser['is_premium'] ?? false
              : false; // Handle case where dbUser might be empty
          _currentUser = _currentUser!.copyWith(isPremium: isPremium);
          debugPrint(
  ...
  app_user.User _convertSupabaseUser(supabase_auth.User user) {
    ...
    final newUser = app_user.User(
      ...
      providerId: user.id,
      targetLanguage: userData['target_language'] ?? 'it',
      nativeLanguage: userData['native_language'] ?? 'en',
  ...
  Future<void> _fetchAndUpdatePremiumStatus(String userId) async {
    try {
      ...
      final isPremium =
          (response.isNotEmpty) ? response['is_premium'] ?? false : false;
      debugPrint(
